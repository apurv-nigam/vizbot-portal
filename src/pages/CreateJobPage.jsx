    ...
    <AppShell>
      <div className="max-w-[560px] mx-auto px-6 lg:px-9 py-6">
        <button
          ...
          className="flex items-center gap-1.5 text-[13px] font-medium text-text-muted hover:text-text-secondary transition-colors cursor-pointer mb-5"
        >
          <ArrowLeft size={14} /> Jobs
        </button>

        <h1 className="text-[24px] font-semibold text-text-primary tracking-[-0.5px] mb-1">Create Job</h1>
        <p className="text-[13px] text-text-secondary mb-6">Select a site and workflow to create a field assignment.</p>

        <Card>
          <CardContent className="p-6 space-y-5">
            {/* Site picker */}
            <div>
              <label className="block text-[12px] font-semibold text-text-secondary mb-1.5">Site *</label>
              {selectedSite ? (
                <div className="flex items-center justify-between h-[42px] px-3.5 rounded-lg border-[1.5px] border-accent bg-surface-hover">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-[13px] font-medium text-text-primary">{selectedSite.name}</span>
                      {selectedSite.code && <span className="ml-1 text-[11px] font-mono text-text-muted">{selectedSite.code}</span>}
                    </div>
                  </div>
                  <button onClick={() => { setSelectedSiteId(""); setSiteSearch(""); }} className="text-[12px] text-text-muted hover:text-text-secondary cursor-pointer">Change</button>
                </div>
              ) : (
                    ...
                    <>
                      ...
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg border border-border shadow-[0_8px_30px_rgba(0,0,0,0.1)] max-h-[240px] overflow-y-auto z-20">
                        {filteredSites.length === 0 ? (
                          <p className="p-3 text-[13px] text-text-muted">No sites found</p>
                        ) : (
                          filteredSites.map((s) => (
                            <button
                              key={s.id}
                            ...
                            >
                              ...
                              <div className="min-w-0">
                                <p className="text-[13px] font-medium text-text-primary truncate">
                                  {s.name}
                                  {s.code && <span className="ml-1.5 text-[11px] font-mono text-text-muted">{s.code}</span>}
                                </p>
                                <p className="text-[11px] text-text-muted truncate">{[s.district, s.state].filter(Boolean).join(", ") || "No location"}</p>
                              </div>
                            </button>
                          ))
                        )}
            ...
            {/* Workflow picker */}
            <div>
              <label className="block text-[12px] font-semibold text-text-secondary mb-1.5">Workflow *</label>
              <div className="relative">
                <select
            ...
            {/* Due date */}
            <div>
              <label className="flex items-center gap-1.5 text-[12px] font-semibold text-text-secondary mb-1.5">
            ...
            {autoTitle && (
              <div className="bg-accent-light rounded-lg px-3.5 py-2.5">
                <p className="text-[11px] font-semibold text-accent-text mb-0.5">Job will be created as:</p>
                <p className="text-[13px] font-medium text-accent-text">{autoTitle}</p>
                {selectedSite?.technician_name && (
                  <p className="text-[11px] text-accent mt-1">Assigned to {selectedSite.technician_name} · Due {new Date(dueDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
                )}
              </div>
            )}
